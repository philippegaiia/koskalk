...
## Generate English guidance before guidance localization
Full ingredient enrichment localizes only the finalized human-facing display and optional saponification names; it never translates guidance and never translates INCI, identifiers, or Latin botanical names. Automatic English guidance generation is disabled for the external-guidance workflow. After the externally authored English guidance is pasted, proofread, and saved, the Guidance & media “Translate guidance” action starts a GuidanceLocalization batch from canonical English. Localization-only translates only guidance, never identity names or other ingredient data. It preserves reviewer-owned locale rows and updates only missing, stale, or incomplete AI/legacy guidance locales.
...
The renderer drops claims matching bounded evidence-workflow prose patterns and records a reviewable warning: catalogue classification; an unnamed `this`/`for this` grade; evidence adjectives qualifying a material, grade, profile, or data; or supplier/manufacturer narration. Explicit named grades and bounded experimental language remain allowed; prompt compliance alone is not trusted.
...
Official identifiers always win. A substance may legitimately carry several accepted CAS or EC numbers, so corroborated values coexist with official ones as non-primary rows; only an exact value the official record already carries is skipped (value-level precedence, owner-confirmed). CAS or EC numbers that official records lack may enter the proposal only when at least two independent consulted research sources explicitly print them — independence is measured by PSL-backed registrable publisher domains, including private suffix rules, never by URL count, so pages on one registrable publisher domain are a single authority. The resolver fails closed when a publisher domain cannot be resolved safely. Entries are tagged with source tier `approved_secondary` and `source_confirmed` provenance listing every corroborating URL. The research model reports identifiers only when a consulted source prints them, never guessing or converting. Stage data read back from PostgreSQL is compared order-insensitively (jsonb canonicalizes object key order); never reintroduce strict array equality on persisted research stage data.

Publisher-domain corroboration uses a locally deployed, integrity-pinned Public Suffix List snapshot, including private suffix rules. Enrichment runtime never downloads the PSL. A missing or corrupt snapshot emits one structured warning per worker process and disables corroboration; no hostname fallback is permitted. Refreshes replace the snapshot and its hash together.

Accepted corroborating evidence rows are authoritative. Proposal editing preserves and re-indexes all prior rows only when the normalized `scheme:value` and inline source attributes are unchanged. If the inline source attributes change, it rebuilds one row from the submitted inline source; a changed or removed identifier inherits none of the prior rows, so stale positional evidence never attaches. On apply, every accepted identity-matched evidence row attaches to the final identifier, with the submitted inline source as fallback only when no accepted rows exist. Validated arrays persist separately at `source_data.enrichment.core.field_confidence` and `source_data.enrichment.core.value_provenance`; neither is inferred from the other.

## Accept external cosmetic-formulation heading
Externally authored English guidance may use `## Cosmetic formulation` as an accepted alias of the canonical `## Formulation use`. Normalize the alias only for validation and completeness checks, preserve the user's original heading text, and validate English structure before spending tokens on localization.
