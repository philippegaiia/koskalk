...
The renderer also drops claims containing evidence-workflow or catalogue-schema prose and records a reviewable warning; prompt compliance alone is not trusted.
...
Accepted corroborating evidence rows are authoritative and must survive unchanged proposal edits and final apply. Proposal edits preserve every row for an unchanged identifier while re-indexing its field; changed or removed identifiers do not inherit old rows, so stale positional evidence never attaches. Applied enrichment metadata persists `field_confidence` and `value_provenance` separately; neither is inferred from the other.
