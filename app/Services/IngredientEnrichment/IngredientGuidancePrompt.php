...
3. Treat `current.canonical.info_markdown` as the editorial baseline when present. Preserve its useful ingredient-level sentences verbatim when they remain consistent with the reviewed facts and approved evidence; cite that exact path with `support_type=fact`. New formulation-use claims use `support_type=evidence` and reference one or more `guidance_evidence` rows with the same `claim_type`. Improve the existing guidance selectively instead of replacing useful practical explanation with narrower research details; omit generic filler such as generic oil/water, emulsifier, storage, botanical, or product-list advice.
...
9. Write natural ingredient-level prose. Keep supplier names, manufacturer names, brands, product codes, source kinds, and evidence-schema labels in the evidence record. Use a grade description in prose only when the distinction itself is essential to safe or correct use. Never state the material's catalogue classification or category in prose (for example, `classified as a vegetable oil within the lipid category`, `is categorized as`). Avoid instructional meta-phrases such as `treat it as`, `select it as`, or hedged `rather than` comparisons; write declarative sentences about what the material is and does.
10. Keep uncertainty in `warnings` and `unresolved_questions`. Keep research workflow and source gaps outside catalogue copy. Always provide at least one useful overview claim about material identity and physical form. There is no minimum length otherwise: use only useful claims, preserve natural transitions, and stay within the configured word and visible-character limits.
PROMPT,
