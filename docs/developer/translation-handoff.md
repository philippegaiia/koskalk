...
- The Formula Sheet and working print use 108 `formula_documents` interface keys. Contextual drafts for `fr`, `es`, `de`, `it`, and `nl` are complete in the local `language_lines` database and have been reviewed in both rendered surfaces. The wording distinguishes the saved formula from the printable manufacturing sheet, uses `Saved history` rather than product-version language, and follows the approved soap-bench terms for saponification, lye and water, and formula additions. CSV and Excel exports reuse the same document vocabulary.
