    ...
    @foreach ([
        ...
        <a
            ...
            @class([
                ...
                'border-[var(--color-accent)] text-[var(--color-ink-strong)]' => $routeName === 'production-bench.purchasing.suppliers' ? request()->routeIs('production-bench.purchasing.*') : ($routeName === 'production-bench.production.index' ? $productionWorkflowActive : request()->routeIs($routeName)),
                'border-transparent text-[var(--color-ink-soft)] hover:text-[var(--color-ink-strong)]' => $routeName === 'production-bench.purchasing.suppliers' ? ! request()->routeIs('production-bench.purchasing.*') : ($routeName === 'production-bench.production.index' ? ! $productionWorkflowActive : ! request()->routeIs($routeName)),
            ])
        >{{ $label }}</a>
    @endforeach
</nav>
