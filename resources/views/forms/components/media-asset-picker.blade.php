    ...
    <div
        x-data="mediaAssetPicker({
            ...
            messages: {
                ...
                download: @js(__('media_library.documents.download')),
            },
        })"
                        ...
                        <div class="flex items-center gap-3 rounded-xl border border-[var(--color-line)] bg-[var(--color-field-muted)] p-2 pr-3">
                            @if ($asset->getFirstMedia('master') && $preserveAspectRatio)
                                <span class="grid aspect-[4/3] w-32 shrink-0 place-items-center overflow-hidden rounded-lg bg-[var(--color-panel-strong)]">
                                    <img src="{{ route('media.show', [$asset, 'recipe-index']) }}" alt="" class="size-full object-contain" draggable="false" />
                                </span>
                ...
                <div class="max-h-[56dvh] overflow-y-auto p-5">
                    <div id="{{ $pickerId }}-library-panel" aria-labelledby="{{ $pickerId }}-library-tab" x-show="activeTab === 'library'" role="tabpanel" class="space-y-4">
                        ...
                        <div data-media-picker-pending-status x-show="pendingUpload" role="status" aria-live="polite" aria-atomic="true" class="rounded-xl border border-[var(--color-line)] bg-[var(--color-accent-soft)] p-4">
                            ...
                            <div x-show="pendingUpload?.status === 'processing' || pendingUpload?.status === 'failed'" class="mt-3 flex gap-2"><button data-media-picker-retry type="button" x-show="pendingUpload?.retryUrl" x-on:click="retryUpload()" class="sk-btn sk-btn-primary">{{ __('media_library.picker.retry') }}</button><button data-media-picker-remove type="button" x-show="pendingUpload?.removeUrl" x-on:click="removeUpload()" class="sk-btn border border-[var(--color-line)]"><span x-show="pendingUpload?.status === 'processing'">{{ __('media_library.picker.cancel') }}</span><span x-show="pendingUpload?.status !== 'processing'">{{ __('media_library.picker.remove') }}</span></button></div>
                        </div>
