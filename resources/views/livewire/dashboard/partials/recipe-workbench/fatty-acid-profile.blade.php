<div class="sk-card sk-tone-analysis overflow-hidden">
 ...
 <span aria-hidden="true" class="pointer-events-none absolute inset-y-1 left-2 right-14 z-10 hidden items-center rounded-md bg-[var(--color-active)] px-3 font-medium text-[var(--color-on-active)] opacity-0 shadow-sm transition-opacity motion-reduce:transition-none lg:flex lg:group-hover/fatty-row:opacity-100" x-text="segment.label"></span>
 </div>
 </template>
 </div>
 </div>
 ...
 <div class="grid gap-1.5 border-t border-[var(--color-line)] px-2 py-3">
 <template x-for="row in fattyAcidProfileRows" :key="row.key">
 <div class="grid grid-cols-[minmax(0,5.5rem)_minmax(3rem,1fr)_3rem] items-center gap-2 rounded-md bg-white/70 px-2 py-2 text-xs">
