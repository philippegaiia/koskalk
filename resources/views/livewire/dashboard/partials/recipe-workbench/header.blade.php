<section class="sk-card p-5">
 ...
 <div class="min-w-0 flex-1">
	 ...
	 <input x-model="formulaName" type="text" :placeholder="isCosmeticFormula ? 'Untitled cosmetic formula' : 'Untitled soap formula'" class="mt-2 w-full rounded-[1.25rem] border border-[var(--color-line)] bg-[var(--color-field)] px-4 py-3 text-2xl font-semibold text-[var(--color-ink-strong)] outline outline-1 outline-[var(--color-field-outline)] transition focus:outline-2 focus:outline-[var(--color-accent)]" />
	 </div>
 ...
 <div x-show="isOfficialSaveModalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @keydown.escape.window="isOfficialSaveModalOpen = false" @click.self="isOfficialSaveModalOpen = false">
 <div class="w-full max-w-lg rounded-lg border border-[var(--color-line)] bg-white p-5 shadow-xl">
 <p class="sk-eyebrow">Official recipe</p>
 <h2 class="mt-2 text-xl font-semibold text-[var(--color-ink-strong)]">Update official recipe?</h2>
 ...
 <button type="button" @click="isOfficialSaveModalOpen = false" class="rounded-lg border border-[var(--color-line)] px-4 py-2 text-sm font-medium text-[var(--color-ink-strong)] transition hover:bg-[var(--color-panel)]">
 Cancel
 </button>
 <button type="button" @click="saveRecipe()" :disabled="isSaving" class="rounded-lg bg-[var(--color-accent-strong)] px-4 py-2 text-sm font-medium text-white transition hover:bg-[var(--color-accent)] disabled:cursor-not-allowed disabled:bg-[var(--color-line)] disabled:text-[var(--color-ink-soft)]">
