...
<section class="sk-card p-5" aria-labelledby="ingredient-list-preview-heading">
 ...
 <div class="mt-5 overflow-hidden sk-inset" x-data="{ overridden: null }">
 <div class="flex flex-col gap-3 border-b border-[var(--color-line)] px-4 py-3 sm:flex-row sm:items-start sm:justify-between" aria-labelledby="declaration-details-heading">
 <div class="min-w-0">
 <h3 id="declaration-details-heading" class="font-medium text-[var(--color-ink-strong)]">{{ __('workbench.output.declarations.title') }}</h3>
 ...
 <button
 type="button"
 :aria-expanded="(overridden ?? declarationRowsRequireAttention).toString()"
 aria-controls="declaration-details-body"
 @click="overridden = !(overridden ?? declarationRowsRequireAttention)"
 ...
 >
 <svg viewBox="0 0 16 16" fill="none" class="size-4 transition-transform duration-200 motion-reduce:transition-none" :class="(overridden ?? declarationRowsRequireAttention) ? 'rotate-180' : ''" aria-hidden="true">
 <path d="M4 6l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
 </svg>
 </button>
 </div>

 <div id="declaration-details-body" class="grid transition-[grid-template-rows,visibility] duration-300 ease-out motion-reduce:transition-none" :class="(overridden ?? declarationRowsRequireAttention) ? 'grid-rows-[1fr]' : 'grid-rows-[0fr] invisible'">
 <div class="overflow-hidden">

 <template x-if="curedSoapDeclarationRows.length > 0">
 <div class="overflow-x-auto">
 <table class="min-w-full divide-y divide-[var(--color-line)] text-sm">
 <caption class="sr-only">{{ __('workbench.output.declarations.title') }}</caption>
 ...
 <th scope="col" class="px-4 py-3">{{ $isCosmeticWorkbench ? __('workbench.output.declarations.formula_percent') : __('workbench.output.common.soap_percent') }}</th>
 <th scope="col" class="px-4 py-3">{{ __('workbench.output.declarations.threshold') }}</th>
 <th scope="col" class="px-4 py-3">{{ __('workbench.output.common.status') }}</th>
 <th scope="col" class="px-4 py-3">{{ __('workbench.output.declarations.notes') }}</th>
 </tr>
 </thead>
 ...
 <span :class="declarationStatusClasses(row)" class="inline-flex justify-center rounded-full border px-3 py-1 text-[0.6875rem] font-medium leading-5" x-text="row.status_label"></span>
 </td>
 <td class="max-w-60 px-4 py-3 text-xs leading-5 [overflow-wrap:anywhere]" x-text="row.notes"></td>
 </tr>
 </template>
 </tbody>
 </table>
 ...
 <p class="border-t border-[var(--color-line)] px-4 py-3 text-xs leading-5 text-[var(--color-ink-soft)]">{{ __('workbench.output.declarations.disclaimer') }}</p>
 </div>
 </div>
 </div>
</section>
