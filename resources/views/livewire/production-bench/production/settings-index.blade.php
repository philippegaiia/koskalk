<x-production-bench.page active="production-setup" :subnavigation="$section">
    ...
    <div class="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,.65fr)]">
        <section aria-labelledby="holiday-heading" class="space-y-4">
            ...
            <form wire:submit="saveHoliday" class="sk-card grid gap-4 p-5 sm:grid-cols-[minmax(0,1fr)_12rem_auto]"><label class="text-sm"><span class="font-medium">{{ __('production_bench.settings.holiday_name') }}</span><input wire:model="holidayName" class="sk-input mt-1 w-full" placeholder="Summer closure" @disabled(! $isBenchActive || $isReadOnly)>@error('holidayName')<span class="mt-1 block text-xs text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>{{ $this->holidayDateForm->getComponent('holidayDate') }}<label class="flex items-center gap-2 self-end pb-2 text-sm"><input wire:model="holidayIsRecurring" type="checkbox" class="size-4 rounded accent-[var(--color-accent)]" @disabled(! $isBenchActive || $isReadOnly)>{{ __('production_bench.settings.recurring') }}</label><button type="submit" class="sk-btn sk-btn-primary sm:col-span-3 sm:justify-self-end" @disabled(! $isBenchActive || $isReadOnly)>{{ __('production_bench.settings.add_holiday') }}</button></form>
