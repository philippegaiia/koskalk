<x-production-bench.page purchasing compact>
    ...
    <section aria-labelledby="receipt-source-heading" class="space-y-3">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
            <button type="button" wire:click="chooseSource('purchase_order')" aria-pressed="{{ $source === 'purchase_order' ? 'true' : 'false' }}" @class(['min-h-20 rounded-xl border p-4 text-left transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-accent)]', 'border-[var(--color-accent)] bg-[var(--color-accent-soft)]' => $source === 'purchase_order', 'border-[var(--color-line)] bg-[var(--color-panel)] hover:bg-[var(--color-panel-strong)]' => $source !== 'purchase_order'])>
            ...
            <button type="button" wire:click="chooseSource('direct')" aria-pressed="{{ $source === 'direct' ? 'true' : 'false' }}" @class(['min-h-20 rounded-xl border p-4 text-left transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-accent)]', 'border-[var(--color-accent)] bg-[var(--color-accent-soft)]' => $source === 'direct', 'border-[var(--color-line)] bg-[var(--color-panel)] hover:bg-[var(--color-panel-strong)]' => $source !== 'direct'])>
        ...
        <form wire:submit="post" class="space-y-6">
            <section class="sk-card grid gap-4 p-5 sm:grid-cols-2">
                ...
                <label class="text-sm">
                    ...
                    <input wire:model="receivedAt" type="date" required aria-invalid="{{ $errors->has('receivedAt') ? 'true' : 'false' }}" class="sk-input mt-1 w-full">
                    @error('receivedAt') <span class="mt-1 block text-xs text-[var(--color-danger-strong)]">{{ $message }}</span> @enderror
                </label>
                <label class="text-sm">
            ...
            @if ($source === 'purchase_order')
                <section class="space-y-4">
                    <label class="block text-sm">
                        ...
                        <select wire:model.live="orderPublicId" required aria-invalid="{{ $errors->has('orderPublicId') ? 'true' : 'false' }}" class="sk-input mt-1 w-full">
                        ...
                        @error('orderPublicId') <span class="mt-1 block text-xs text-[var(--color-danger-strong)]">{{ $message }}</span> @enderror
                    </label>

                    @if ($selectedOrder)
                        <p class="text-xs text-[var(--color-ink-soft)]">{{ __('production_bench.receipt.currency_locked_help') }}</p>
                        <div class="overflow-x-auto" data-receipt-responsive-table>
                            <div class="min-w-[980px] space-y-2">
                                <div class="grid grid-cols-[2rem_minmax(12rem,1.4fr)_6rem_7rem_6rem_minmax(29rem,2fr)] bg-[var(--color-panel-muted)] text-xs uppercase tracking-wide text-[var(--color-ink-soft)]">
                                ...
                                @foreach ($selectedOrder->lines as $line)
                                    ...
                                    <article wire:key="order-receipt-line-{{ $line->id }}" class="grid grid-cols-[2rem_minmax(12rem,1.4fr)_6rem_7rem_6rem_minmax(29rem,2fr)] items-start rounded-xl bg-[var(--color-field-muted)] text-sm">
                                        ...
                                        <div class="px-3 py-3"><p class="font-medium text-[var(--color-ink-strong)]">{{ $line->ingredient?->localizedDisplayName() ?? $line->packagingItem?->name }}</p><p class="mt-1 text-xs text-[var(--color-ink-soft)]">{{ $line->listing_name }}</p></div>
                                        <span class="numeric px-3 py-3 text-right">{{ $line->ordered_packs }}</span><span class="numeric px-3 py-3 text-right">{{ $posted }}</span><span class="numeric px-3 py-3 text-right">{{ $remaining }}</span>
                                        <div class="grid gap-2 px-3 py-3 sm:grid-cols-6">
            ...
            @else
                <section class="space-y-4">
                    <label class="block text-sm"><span class="font-medium">{{ __('production_bench.supplier.singular') }}</span><select wire:model.live="supplierId" required class="sk-input mt-1 w-full"><option value="">{{ __('production_bench.receipt.choose_supplier') }}</option>@foreach($suppliers as $supplier)<option value="{{ $supplier->id }}">{{ $supplier->name }}</option>@endforeach</select>@error('supplierId') <span class="mt-1 block text-xs text-[var(--color-danger-strong)]">{{ $message }}</span> @enderror</label>
                    @if ($supplierId)
                        <p class="text-xs text-[var(--color-ink-soft)]">{{ __('production_bench.receipt.currency_locked_help') }}</p>
                        <div class="space-y-2">
            ...
            @if ($errors->any())
                <div role="alert" class="rounded-xl bg-[var(--color-danger-soft)] px-4 py-3 text-sm text-[var(--color-danger-strong)]">
            ...
            <div class="pb-24">
                <x-workflow-action-bar data-receipt-action-bar>
                    <a href="{{ route('production-bench.purchasing.receipts') }}" wire:navigate class="sk-btn sk-btn-ghost">{{ __('production_bench.common.cancel') }}</a>
                    <button type="submit" wire:confirm="{{ __('production_bench.receipt.post_confirm') }}" class="sk-btn sk-btn-primary">{{ __('production_bench.receipt.post') }}</button>
