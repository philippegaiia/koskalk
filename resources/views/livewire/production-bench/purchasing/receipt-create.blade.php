                        ...
                        <div class="space-y-3" data-receipt-editable-lines>
                            @foreach ($selectedOrder->lines as $line)
                                ...
                                <article wire:key="order-receipt-line-{{ $line->id }}" data-receipt-mobile-context aria-disabled="{{ $remaining < 1 ? 'true' : 'false' }}" @class(['rounded-xl border border-[var(--color-line)] p-4 text-sm', 'bg-[var(--color-field-muted)]' => $remaining > 0, 'bg-[var(--color-panel-muted)]' => $remaining < 1])>
                                    <fieldset data-receipt-line-fields="{{ $line->id }}" @disabled($remaining < 1) class="space-y-4">
                                        <legend class="sr-only">{{ $line->ingredient?->localizedDisplayName() ?? $line->packagingItem?->name }}</legend>
                                        <div data-receipt-line-summary="{{ $line->id }}" class="flex flex-col gap-4 lg:flex-row lg:items-end">
                                            <div class="flex min-w-0 flex-1 items-start gap-3">
                                                <label class="flex min-h-11 items-center"><span class="sr-only">{{ __('production_bench.receipt.select_item', ['item' => $line->listing_name]) }}</span><input wire:model.live="selected.{{ $line->id }}" type="checkbox" @disabled($remaining < 1) class="size-5 rounded border-[var(--color-line-strong)] text-[var(--color-accent)] focus:ring-[var(--color-accent)] disabled:cursor-not-allowed disabled:opacity-40"></label>
                                            ...
                                            <dl class="grid grid-cols-3 gap-3 lg:w-[22rem]">
                                            ...
                                            <label class="block text-xs lg:w-44">
                                                {{ __('production_bench.receipt.formats') }}
                                                @if($remaining > 0)
                                                    <input wire:model="lineInputs.{{ $line->id }}.packs_received" type="number" min="1" max="{{ $remaining }}" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.packs_received") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.packs_received")) aria-describedby="line-{{ $line->id }}-packs-error" @endif class="sk-input mt-1 py-2 numeric">
                                                @else
                                                    <input type="number" value="0" disabled class="sk-input mt-1 cursor-not-allowed bg-[var(--color-panel-muted)] py-2 numeric text-[var(--color-ink-muted)] opacity-60">
                                                @endif
                                        ...
                                        <div data-receipt-line-details="{{ $line->id }}" @class(['grid gap-3 border-t border-[var(--color-line)] pt-4 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-[minmax(11rem,1.3fr)_minmax(7rem,.8fr)_minmax(11rem,1.1fr)_minmax(6rem,.6fr)_minmax(9rem,1fr)_minmax(11rem,1.1fr)_minmax(9rem,1fr)]', 'opacity-50' => $remaining < 1])>
                                            ...
                                            <label class="text-xs">{{ __('production_bench.receipt.price_basis') }}<select wire:model="lineInputs.{{ $line->id }}.receipt_price_basis" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.receipt_price_basis") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.receipt_price_basis")) aria-describedby="line-{{ $line->id }}-price-basis-error" @endif class="sk-input mt-1 py-2"><option value="total_purchase_format">{{ __('production_bench.receipt.total_format') }}</option><option value="per_unit">{{ __('production_bench.receipt.per_unit') }}</option></select>@error("lineInputs.{$line->id}.receipt_price_basis")<span id="line-{{ $line->id }}-price-basis-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                            <label class="text-xs">{{ __('production_bench.receipt.price') }}<span class="mt-1 flex gap-1"><input wire:model="lineInputs.{{ $line->id }}.receipt_price_amount" inputmode="decimal" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.receipt_price_amount") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.receipt_price_amount")) aria-describedby="line-{{ $line->id }}-price-error" @endif class="sk-input min-w-0 flex-1 py-2 numeric"><input wire:model="lineInputs.{{ $line->id }}.currency" maxlength="3" readonly data-receipt-currency-locked aria-label="{{ __('production_bench.common.currency') }}" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.currency") ? 'true' : 'false' }}" aria-describedby="receipt-order-currency-help{{ $errors->has("lineInputs.{$line->id}.currency") ? ' line-'.$line->id.'-currency-error' : '' }}" class="sk-input w-16 py-2 uppercase"></span><span class="mt-1 block text-[10px] leading-4 text-[var(--color-ink-muted)]">{{ __('production_bench.receipt.inventory_cost_after_posting', ['currency' => $workspaceCurrency]) }}</span>@error("lineInputs.{$line->id}.receipt_price_amount")<span id="line-{{ $line->id }}-price-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror @error("lineInputs.{$line->id}.currency")<span id="line-{{ $line->id }}-currency-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                            <label class="text-xs">{{ __('production_bench.receipt.price_unit') }}<input wire:model="lineInputs.{{ $line->id }}.receipt_price_unit" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.receipt_price_unit") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.receipt_price_unit")) aria-describedby="line-{{ $line->id }}-price-unit-error" @endif class="sk-input mt-1 py-2">@error("lineInputs.{$line->id}.receipt_price_unit")<span id="line-{{ $line->id }}-price-unit-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                            <label class="text-xs">{{ __('production_bench.receipt.supplier_batch') }}<input wire:model="lineInputs.{{ $line->id }}.supplier_batch_number" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.supplier_batch_number") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.supplier_batch_number")) aria-describedby="line-{{ $line->id }}-batch-error" @endif class="sk-input mt-1 py-2">@error("lineInputs.{$line->id}.supplier_batch_number")<span id="line-{{ $line->id }}-batch-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                            <label class="text-xs">{{ __('production_bench.receipt.expiry') }}<input wire:model="lineInputs.{{ $line->id }}.expires_at" type="date" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.expires_at") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.expires_at")) aria-describedby="line-{{ $line->id }}-expiry-error" @endif class="sk-input mt-1 py-2">@error("lineInputs.{$line->id}.expires_at")<span id="line-{{ $line->id }}-expiry-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                            <label class="text-xs">{{ __('production_bench.common.notes') }}<input wire:model="lineInputs.{{ $line->id }}.notes" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.notes") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.notes")) aria-describedby="line-{{ $line->id }}-notes-error" @endif class="sk-input mt-1 py-2">@error("lineInputs.{$line->id}.notes")<span id="line-{{ $line->id }}-notes-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                        </div>
                                    </fieldset>
                                </article>
                            @endforeach
