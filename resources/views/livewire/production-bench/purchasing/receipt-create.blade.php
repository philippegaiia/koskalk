    ...
    @if ($source === 'purchase_order' || $source === 'direct')
        <form wire:submit="post" wire:loading.attr="aria-busy" wire:target="post" class="space-y-6">
            <section class="sk-card grid gap-4 p-5 sm:grid-cols-2">
                ...
                {{ $this->receiptDatesForm->getComponent('received_at') }}
                <label class="text-sm">
                                        ...
                                        <div data-receipt-line-grid="{{ $line->id }}">
                                            ...
                                            <div data-receipt-line-details="{{ $line->id }}" @class(['grid gap-x-5 gap-y-4 border-t border-[var(--color-line)] py-5 sm:grid-cols-2 xl:grid-cols-4', 'opacity-50' => $remaining < 1])>
                                                ...
                                                @if (strtoupper((string) $line->currency) !== strtoupper((string) $workspaceCurrency))
                                                    <label class="flex min-w-0 flex-col gap-2 text-xs">{{ __('production_bench.receipt.manual_exchange_rate') }}<input wire:model="lineInputs.{{ $line->id }}.manual_exchange_rate" inputmode="decimal" data-receipt-manual-rate="{{ $line->id }}" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.manual_exchange_rate") ? 'true' : 'false' }}" class="sk-input w-full py-2 numeric"><span class="text-[10px] leading-4 text-[var(--color-ink-muted)]">{{ __('production_bench.receipt.manual_exchange_rate_help') }}</span>@if ($this->autoRateUnavailable($line->currency))<span class="text-[10px] leading-4 font-medium text-[var(--color-warning-strong)]">{{ __('production_bench.receipt.auto_rate_unavailable_hint') }}</span>@endif@error("lineInputs.{$line->id}.manual_exchange_rate")<span class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                                @endif
                                            </div>
                                            ...
                                            <div data-receipt-line-metadata="{{ $line->id }}" @class(['grid gap-x-5 gap-y-4 border-t border-[var(--color-line)] pt-5 sm:grid-cols-2 xl:grid-cols-4', 'opacity-50' => $remaining < 1])>
                                                @if ($line->ingredient_id !== null)
                                                    <label class="text-sm"><span class="font-medium">{{ __('lot_numbering.manual_label') }}</span><input wire:model="lineInputs.{{ $line->id }}.internal_lot_code" maxlength="64" placeholder="{{ __('lot_numbering.automatic') }}" aria-describedby="line-{{ $line->id }}-internal-lot-help" class="sk-input mt-1 w-full"><span id="line-{{ $line->id }}-internal-lot-help" class="mt-1 block text-xs text-[var(--color-ink-soft)]">{{ __('lot_numbering.manual_help') }}</span>@error("lineInputs.{$line->id}.internal_lot_code")<span class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                                @endif
                                                ...
                                                {{ $this->receiptDatesForm->getComponent("expiry_date_{$line->id}") }}
                                ...
                                <article wire:key="direct-receipt-line-{{ $listing->id }}" class="rounded-xl bg-[var(--color-field-muted)] p-4">
                                    ...
                                    <div class="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                                        ...
                                        @if (strtoupper((string) $listing->currency) !== strtoupper((string) $workspaceCurrency))
                                            <label class="flex min-w-0 flex-col gap-2 text-xs">{{ __('production_bench.receipt.manual_exchange_rate') }}<input wire:model="lineInputs.{{ $listing->id }}.manual_exchange_rate" inputmode="decimal" data-receipt-manual-rate="{{ $listing->id }}" aria-invalid="{{ $errors->has("lineInputs.{$listing->id}.manual_exchange_rate") ? 'true' : 'false' }}" class="sk-input w-full py-2 numeric"><span class="text-[10px] leading-4 text-[var(--color-ink-muted)]">{{ __('production_bench.receipt.manual_exchange_rate_help') }}</span>@if ($this->autoRateUnavailable($listing->currency))<span class="text-[10px] leading-4 font-medium text-[var(--color-warning-strong)]">{{ __('production_bench.receipt.auto_rate_unavailable_hint') }}</span>@endif@error("lineInputs.{$listing->id}.manual_exchange_rate")<span class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                        @endif
                                        @if ($listing->ingredient_id !== null)
                                            <label class="text-sm"><span class="font-medium">{{ __('lot_numbering.manual_label') }}</span><input wire:model="lineInputs.{{ $listing->id }}.internal_lot_code" maxlength="64" placeholder="{{ __('lot_numbering.automatic') }}" aria-describedby="line-{{ $listing->id }}-internal-lot-help" class="sk-input mt-1 w-full"><span id="line-{{ $listing->id }}-internal-lot-help" class="mt-1 block text-xs text-[var(--color-ink-soft)]">{{ __('lot_numbering.manual_help') }}</span>@error("lineInputs.{$listing->id}.internal_lot_code")<span class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                        @endif
                                        ...
                                        {{ $this->receiptDatesForm->getComponent("expiry_date_{$listing->id}") }}
