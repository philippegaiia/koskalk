                                    ...
                                    <fieldset data-receipt-line-fields="{{ $line->id }}" @disabled($remaining < 1)>
                                        ...
                                        <div data-receipt-line-grid="{{ $line->id }}">
                                            ...
                                            <div data-receipt-line-details="{{ $line->id }}" @class(['grid gap-x-5 gap-y-4 border-t border-[var(--color-line)] py-5 sm:grid-cols-2 xl:grid-cols-4', 'opacity-50' => $remaining < 1])>
                                                ...
                                                <label class="flex min-w-0 flex-col gap-2 text-xs">{{ __('production_bench.receipt.price_basis') }}<select wire:model="lineInputs.{{ $line->id }}.receipt_price_basis" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.receipt_price_basis") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.receipt_price_basis")) aria-describedby="line-{{ $line->id }}-price-basis-error" @endif class="sk-input w-full py-2"><option value="total_purchase_format">{{ __('production_bench.receipt.total_format') }}</option><option value="per_unit">{{ __('production_bench.receipt.per_unit') }}</option></select>@error("lineInputs.{$line->id}.receipt_price_basis")<span id="line-{{ $line->id }}-price-basis-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                                ...
                                                <label class="flex min-w-0 flex-col gap-2 text-xs">{{ __('production_bench.receipt.price_unit') }}<input wire:model="lineInputs.{{ $line->id }}.receipt_price_unit" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.receipt_price_unit") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.receipt_price_unit")) aria-describedby="line-{{ $line->id }}-price-unit-error" @endif class="sk-input w-full py-2">@error("lineInputs.{$line->id}.receipt_price_unit")<span id="line-{{ $line->id }}-price-unit-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                            </div>
