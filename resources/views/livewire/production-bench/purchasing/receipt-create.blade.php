                    ...
                    @if ($selectedOrder)
                        ...
                        <div class="space-y-3" data-receipt-editable-lines>
                            @foreach ($selectedOrder->lines as $line)
                                ...
                                <article wire:key="order-receipt-line-{{ $line->id }}" data-receipt-mobile-context aria-disabled="{{ $remaining < 1 ? 'true' : 'false' }}" @class(['rounded-xl border border-[var(--color-line)] p-4 text-sm', 'bg-[var(--color-field-muted)]' => $remaining > 0, 'bg-[var(--color-panel-muted)]' => $remaining < 1])>
                                    <fieldset data-receipt-line-fields="{{ $line->id }}" @disabled($remaining < 1)>
                                        ...
                                        <div data-receipt-line-grid="{{ $line->id }}">
                                            <div data-receipt-line-summary="{{ $line->id }}" class="grid gap-x-4 gap-y-3 pb-4 sm:grid-cols-2 xl:grid-cols-[minmax(12rem,.9fr)_repeat(4,minmax(0,1fr))] xl:items-end">
                                                ...
                                                <label class="block min-w-0 text-xs">
                                                    {{ __('production_bench.receipt.formats') }}
                                                    @if($remaining > 0)
                                                        <input wire:model="lineInputs.{{ $line->id }}.packs_received" type="number" min="1" max="{{ $remaining }}" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.packs_received") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.packs_received")) aria-describedby="line-{{ $line->id }}-packs-error" @endif class="sk-input mt-1 w-full py-2 numeric">
                                                    @else
                                                        <input type="number" value="0" disabled class="sk-input mt-1 w-full cursor-not-allowed bg-[var(--color-panel-muted)] py-2 numeric text-[var(--color-ink-muted)] opacity-60">
                                                    @endif
                                            ...
                                            <div data-receipt-line-details="{{ $line->id }}" @class(['grid gap-x-4 gap-y-3 border-t border-[var(--color-line)] py-4 sm:grid-cols-2 xl:grid-cols-[minmax(12rem,.9fr)_repeat(4,minmax(0,1fr))]', 'opacity-50' => $remaining < 1])>
                                                <span aria-hidden="true" class="hidden xl:block"></span>
                                                <label class="min-w-0 text-xs">{{ __('production_bench.receipt.actual_quantity') }}<span class="mt-1 flex gap-1"><input wire:model="lineInputs.{{ $line->id }}.actual_quantity" inputmode="decimal" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.actual_quantity") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.actual_quantity")) aria-describedby="line-{{ $line->id }}-actual-quantity-error" @endif class="sk-input min-w-0 flex-1 py-2 numeric"><select wire:model="lineInputs.{{ $line->id }}.actual_unit" aria-label="{{ __('production_bench.receipt.actual_quantity') }} ({{ __('production_bench.procurement.unit') }})" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.actual_unit") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.actual_unit")) aria-describedby="line-{{ $line->id }}-actual-unit-error" @endif class="sk-input w-20 shrink-0 py-2">@if($line->unit_kind->value === 'count')<option value="count">{{ __('production_bench.receipt.count') }}</option>@else<option value="g">g</option><option value="kg">kg</option><option value="oz">oz</option><option value="lb">lb</option>@endif</select></span>@error("lineInputs.{$line->id}.actual_quantity")<span id="line-{{ $line->id }}-actual-quantity-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror @error("lineInputs.{$line->id}.actual_unit")<span id="line-{{ $line->id }}-actual-unit-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                                <label class="min-w-0 text-xs">{{ __('production_bench.receipt.price_basis') }}<select wire:model="lineInputs.{{ $line->id }}.receipt_price_basis" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.receipt_price_basis") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.receipt_price_basis")) aria-describedby="line-{{ $line->id }}-price-basis-error" @endif class="sk-input mt-1 w-full py-2"><option value="total_purchase_format">{{ __('production_bench.receipt.total_format') }}</option><option value="per_unit">{{ __('production_bench.receipt.per_unit') }}</option></select>@error("lineInputs.{$line->id}.receipt_price_basis")<span id="line-{{ $line->id }}-price-basis-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                                <label class="min-w-0 text-xs">{{ __('production_bench.receipt.price') }}<span class="mt-1 flex gap-1"><input wire:model="lineInputs.{{ $line->id }}.receipt_price_amount" inputmode="decimal" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.receipt_price_amount") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.receipt_price_amount")) aria-describedby="line-{{ $line->id }}-price-error" @endif class="sk-input min-w-0 flex-1 py-2 numeric"><input wire:model="lineInputs.{{ $line->id }}.currency" maxlength="3" readonly data-receipt-currency-locked aria-label="{{ __('production_bench.common.currency') }}" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.currency") ? 'true' : 'false' }}" aria-describedby="receipt-order-currency-help{{ $errors->has("lineInputs.{$line->id}.currency") ? ' line-'.$line->id.'-currency-error' : '' }}" class="sk-input w-16 shrink-0 py-2 uppercase"></span><span class="mt-1 block text-[10px] leading-4 text-[var(--color-ink-muted)]">{{ __('production_bench.receipt.inventory_cost_after_posting', ['currency' => $workspaceCurrency]) }}</span>@error("lineInputs.{$line->id}.receipt_price_amount")<span id="line-{{ $line->id }}-price-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror @error("lineInputs.{$line->id}.currency")<span id="line-{{ $line->id }}-currency-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                                <label class="min-w-0 text-xs">{{ __('production_bench.receipt.price_unit') }}<input wire:model="lineInputs.{{ $line->id }}.receipt_price_unit" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.receipt_price_unit") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.receipt_price_unit")) aria-describedby="line-{{ $line->id }}-price-unit-error" @endif class="sk-input mt-1 w-full py-2">@error("lineInputs.{$line->id}.receipt_price_unit")<span id="line-{{ $line->id }}-price-unit-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                            </div>

                                            <div data-receipt-line-metadata="{{ $line->id }}" @class(['grid gap-x-4 gap-y-3 border-t border-[var(--color-line)] pt-4 sm:grid-cols-2 xl:grid-cols-[minmax(12rem,.9fr)_repeat(4,minmax(0,1fr))]', 'opacity-50' => $remaining < 1])>
                                                <span aria-hidden="true" class="hidden xl:block"></span>
                                                <label class="min-w-0 text-xs">{{ __('production_bench.receipt.supplier_batch') }}<input wire:model="lineInputs.{{ $line->id }}.supplier_batch_number" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.supplier_batch_number") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.supplier_batch_number")) aria-describedby="line-{{ $line->id }}-batch-error" @endif class="sk-input mt-1 w-full py-2">@error("lineInputs.{$line->id}.supplier_batch_number")<span id="line-{{ $line->id }}-batch-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                                <label class="min-w-0 text-xs">{{ __('production_bench.receipt.expiry') }}<input wire:model="lineInputs.{{ $line->id }}.expires_at" type="date" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.expires_at") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.expires_at")) aria-describedby="line-{{ $line->id }}-expiry-error" @endif class="sk-input mt-1 w-full py-2">@error("lineInputs.{$line->id}.expires_at")<span id="line-{{ $line->id }}-expiry-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                                <label class="min-w-0 text-xs xl:col-span-2">{{ __('production_bench.common.notes') }}<input wire:model="lineInputs.{{ $line->id }}.notes" aria-invalid="{{ $errors->has("lineInputs.{$line->id}.notes") ? 'true' : 'false' }}" @if($errors->has("lineInputs.{$line->id}.notes")) aria-describedby="line-{{ $line->id }}-notes-error" @endif class="sk-input mt-1 w-full py-2">@error("lineInputs.{$line->id}.notes")<span id="line-{{ $line->id }}-notes-error" class="mt-1 block text-[var(--color-danger-strong)]">{{ $message }}</span>@enderror</label>
                                            </div>
                                        </div>
                                    </fieldset>
                                </article>
