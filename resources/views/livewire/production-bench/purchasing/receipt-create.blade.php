            ...
            @if ($source === 'purchase_order')
                <section aria-labelledby="receipt-order-lines-heading" class="space-y-4">
                    ...
                    @if ($selectedOrder)
                        ...
                        <div class="overflow-x-auto rounded-xl border border-[var(--color-line)] bg-[var(--color-panel)] p-2" data-receipt-editable-lines>
                            <div class="min-w-[1180px] space-y-2">
                                <div class="grid grid-cols-[2rem_16rem_6rem_8rem_7rem_minmax(44rem,1fr)] bg-[var(--color-panel-muted)] text-xs uppercase tracking-wide text-[var(--color-ink-soft)]">
                                ...
                                @foreach ($selectedOrder->lines as $line)
                                    ...
                                    <article wire:key="order-receipt-line-{{ $line->id }}" data-receipt-mobile-context class="grid grid-cols-[2rem_16rem_6rem_8rem_7rem_minmax(44rem,1fr)] items-start gap-0 rounded-xl bg-[var(--color-field-muted)] p-0 text-sm">
                                        ...
                                        <span class="numeric px-3 py-3 text-right">{{ $line->ordered_packs }}</span>
                                        <span class="numeric px-3 py-3 text-right">{{ $posted }}</span>
                                        <span class="numeric px-3 py-3 text-right">{{ $remaining }}</span>
                                        <div class="grid grid-cols-6 gap-2 px-3 py-3">
                                            ...
                                            <div class="col-span-2 rounded-lg border border-dashed border-[var(--color-line-strong)] px-3 py-2"><span class="block text-xs">{{ __('production_bench.receipt.inventory_cost') }}</span><span class="numeric mt-1 block text-xs text-[var(--color-ink-soft)]">{{ __('production_bench.receipt.inventory_cost_after_posting', ['currency' => $workspaceCurrency]) }}</span></div>
                                        </div>
                                    </article>
                                @endforeach
                            </div>
