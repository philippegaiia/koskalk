                ...
                @foreach ($receipt->lines as $line)
                    <article wire:key="receipt-detail-line-{{ $line->id }}" class="rounded-xl bg-[var(--color-field-muted)] p-4">
                        <div class="grid gap-4 lg:grid-cols-[minmax(13rem,1.2fr)_repeat(4,minmax(7rem,1fr))]">
                            ...
                            <div><p class="sk-eyebrow">{{ __('production_bench.receipt.inventory_cost') }}</p><p class="numeric mt-1 text-sm">{{ \App\Support\NumberLocale::formatAdaptiveDecimal($line->costing_total_cost ?? $line->historical_total_cost, 2, 4) }} {{ $line->costing_currency ?? $line->currency }}</p><p class="numeric mt-1 text-xs text-[var(--color-ink-soft)]">{{ \App\Support\NumberLocale::formatAdaptiveDecimal($line->stockLot->costing_unit_cost ?? $line->stockLot->historical_unit_cost, 2, 6) }} / {{ $line->stockLot->unit_kind->value === 'mass' ? 'g' : __('production_bench.receipt.count') }}</p></div>
                        </div>
