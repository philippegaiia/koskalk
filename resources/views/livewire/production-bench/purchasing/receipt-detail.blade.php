                ...
                @foreach ($receipt->lines as $line)
                    <article wire:key="receipt-detail-line-{{ $line->id }}" data-receipt-detail-line="{{ $line->id }}" class="sk-card-elevation-subtle rounded-xl border border-[var(--color-line)] bg-[var(--color-panel)] p-5">
                        <div class="grid gap-4 lg:grid-cols-[minmax(13rem,1.2fr)_repeat(4,minmax(7rem,1fr))]">
                            ...
                            <div><p class="sk-eyebrow">{{ __('production_bench.receipt.inventory_cost') }}</p><p class="numeric mt-2 text-sm">{{ \App\Support\NumberLocale::formatAdaptiveDecimal($line->costing_total_cost ?? $line->historical_total_cost, 2, 3) }} {{ $line->costing_currency ?? $line->currency }}</p>@if($line->stockLot->unit_kind->value === 'count')<p class="numeric mt-2 text-xs text-[var(--color-ink-soft)]">{{ \App\Support\NumberLocale::formatAdaptiveDecimal($line->stockLot->costing_unit_cost ?? $line->stockLot->historical_unit_cost, 2, 3) }} / {{ __('production_bench.receipt.count') }}</p>@endif</div>
                        </div>
