                            ...
                            <tr wire:key="receipt-{{ $receipt->id }}" class="transition hover:bg-[var(--color-panel-strong)]">
                                ...
                                <td class="numeric px-4 py-4">
                                    @if ($receipt->purchaseOrder)
                                        <a href="{{ route('production-bench.purchasing.procurement.show', $receipt->purchaseOrder) }}" wire:navigate class="font-medium text-[var(--color-accent-strong)] hover:underline">{{ $receipt->purchaseOrder->reference }}</a>
                                    @else
                                ...
                                <td class="px-5 py-4 text-right"><a href="{{ route('production-bench.purchasing.receipts.show', $receipt) }}" wire:navigate class="font-medium text-[var(--color-accent-strong)] hover:underline">{{ __('production_bench.receipt.open') }}</a></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
