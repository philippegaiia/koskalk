...
it('exposes practical controls for choosing and reordering cosmetic phases', function () {
    ...
    expect($ingredientBrowser)
        ...
        ->toContain('<span aria-hidden="true">↓</span>')
        ->not->toContain('<p class="sk-eyebrow">Phase</p>')
        ->toContain('data-formula-total')
...
it('keeps the cosmetic workbench layout compact and table aligned', function () {
    ...
    expect($header)
        ...
        ->and($settings)
        ->toContain('<div data-cosmetic-primary-settings class="grid gap-4 lg:grid-cols-2">')
        ->toContain('class="sk-inset sk-tone-info p-4" data-formula-output-type')
        ->toContain('data-formula-output-ingredient-fields')
        ->toContain('sk-inset grid gap-4 p-4 lg:col-span-2 lg:grid-cols-2')
        ->toContain('data-product-category-setting')
        ->toContain('mt-3 max-w-3xl')
